    ...
    <div class="dash-scroll">
      ...
      <header class="dash-header">
        <div class="dash-header-mobile">
          ...
          <button class="notif-btn" :class="{ 'notif-btn--dot': hasNotif }">
        ...
        <div v-if="user && !user.is_verified" class="verify-banner">
          ...
          <router-link to="/verify-email" class="verify-link t-mono">Verify →</router-link>
        </div>
      </header>
      ...
      <template v-else>
        ...
        <!-- ── Overspent nudge ── -->
        <div v-if="overspentBudgets.length > 0" class="dash-section" style="padding-bottom:0">
          <div class="dash-nudge">
            ...
            <span class="t-mono" style="font-size:9px">
              {{ overspentBudgets.map((b: any) => b.label).join(', ') }}
              {{ overspentBudgets.length === 1 ? 'budget' : 'budgets' }} exceeded this month.
            </span>
          </div>
        </div>

        <!-- ── Net Worth Card ── -->
        <section class="dash-section sov-fade-up">
          <div class="nw-card">
            <div class="nw-glow" />
            <div class="nw-top">
              <span class="t-mono nw-label">// NET WORTH</span>
              <div style="display:flex;align-items:center;gap:8px">
                <span class="t-mono nw-savings-rate">SAVINGS {{ savingsRate }}%</span>
            ...
            <div class="nw-meta-row">
              <div class="nw-delta" :class="summary.netWorthDelta >= 0 ? 'delta--pos' : 'delta--neg'">
                <svg width="9" height="9" viewBox="0 0 9 9" fill="none">
                  <path :d="summary.netWorthDelta >= 0 ? 'M4.5 7.5V1.5M1.5 4.5l3-3 3 3' : 'M4.5 1.5v6M1.5 4.5l3 3 3-3'"
        ...
        <!-- ── Account Cards ── -->
        <section class="dash-section sov-fade-up sov-fade-up--d1">
          <div class="section-hdr">
            <span class="t-mono section-tag">// ACCOUNTS & WALLETS</span>
            <router-link to="/dashboard/accounts" class="section-link t-mono">Manage →</router-link>
          </div>

          <div v-if="activeAccounts.length === 0" class="empty-card">
            <span class="t-mono empty-label">No accounts yet</span>
            <router-link to="/dashboard/accounts" class="t-mono empty-link">Add one →</router-link>
          </div>

          <template v-else>
            <div class="cards-scroll">
              <div v-for="(card, i) in activeAccounts" :key="card.id"
                class="bank-card" :class="`bank-card--${card.theme ?? card.account_type}`"
                @click="activeCard = i">
              ...
              <div v-if="hasMoreAccounts"
                class="bank-card bank-card--overflow"
                @click="$router.push('/dashboard/accounts')">
                <div class="card-overflow-inner">
                  <div class="card-overflow-icon">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                      <circle cx="10" cy="10" r="8" stroke="currentColor" stroke-width="1.3"/>
                      <path d="M7 10h6M10 7l3 3-3 3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </div>
                  <span class="t-mono card-overflow-label">VIEW ALL</span>
                  <span class="t-mono card-overflow-count">{{ activeCount }} accounts</span>
                </div>
              </div>
            ...
            <div class="cards-dots">
              <span v-for="(_, i) in activeAccounts" :key="i"
                class="cards-dot" :class="{ 'cards-dot--active': activeCard === i }"
                @click="activeCard = i" />
            </div>
          </template>
        </section>
        ...
        <!-- ── Quick Stats ── -->
        <section class="dash-section sov-fade-up sov-fade-up--d2">
          <div class="stats-grid">
            <div class="stat-card">
              <div class="stat-card-top">
                ...
                <span class="stat-trend t-mono" :class="summary.incomeTrend >= 0 ? 'trend--pos' : 'trend--neg'">
                  {{ summary.incomeTrend >= 0 ? '▲' : '▼' }} {{ Math.abs(summary.incomeTrend) }}%
                </span>
              </div>
            ...
            <div class="stat-card">
              <div class="stat-card-top">
                ...
                <span class="stat-trend t-mono" :class="summary.expenseTrend <= 0 ? 'trend--pos' : 'trend--neg'">
                  {{ summary.expenseTrend >= 0 ? '▲' : '▼' }} {{ Math.abs(summary.expenseTrend) }}%
                </span>
              </div>
            ...
            <div class="stat-card stat-card--wide">
              <div class="stat-card-top">
                ...
                <span class="stat-trend t-mono" :class="summary.savingsTrend >= 0 ? 'trend--pos' : 'trend--neg'">
                  {{ summary.savingsTrend >= 0 ? '▲' : '▼' }} {{ Math.abs(summary.savingsTrend) }}%
                </span>
              </div>
        ...
        <!-- ════════════════════════════════════════
             TODAY'S TRANSACTIONS — priority section
        ════════════════════════════════════════ -->
        <section class="dash-section sov-fade-up sov-fade-up--d2">
          <div class="section-hdr">
            <div class="today-hdr-left">
              <span class="t-mono section-tag">// TODAY</span>
              <div v-if="todayTransactions.length > 0" class="today-live-badge">
                <span class="sov-pulse" />
                <span class="t-mono" style="font-size:8px;letter-spacing:1px;color:var(--color-ox)">
                  {{ todayTransactions.length }}
                </span>
              </div>
            </div>
            <div style="display:flex;align-items:center;gap:12px">
              <span v-if="todayTransactions.length > 0"
                class="t-mono today-net"
                :class="todayNet >= 0 ? 'today-net--pos' : 'today-net--neg'">
                {{ todayNet >= 0 ? '+' : '' }}{{ formatAmount(Math.abs(todayNet)) }}
              </span>
              <router-link to="/dashboard/transactions?new=1" class="section-link t-mono">+ Log →</router-link>
            </div>
          </div>

          <!-- Empty today -->
          <div v-if="todayTransactions.length === 0" class="today-empty sov-card">
            <div class="today-empty-inner">
              <div class="today-empty-icon">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                  <circle cx="10" cy="10" r="8" stroke="rgba(224,224,224,0.1)" stroke-width="1.2"/>
                  <path d="M10 6v5l3 3" stroke="rgba(224,224,224,0.15)" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </div>
              <div>
                <p class="t-mono" style="font-size:10px;color:rgba(224,224,224,0.35);letter-spacing:0.5px">No transactions today</p>
                <p class="t-mono" style="font-size:8.5px;color:var(--color-gun);margin-top:2px">Your activity will appear here in real time.</p>
              </div>
            </div>
            <router-link to="/dashboard/transactions?new=1" class="t-mono" style="font-size:9px;color:var(--color-ox);white-space:nowrap">Log one →</router-link>
          </div>

          <!-- Today's list -->
          <div v-else class="txn-list sov-card">
            <div v-for="txn in todayTransactions" :key="txn.id" class="txn-row txn-row--today">
              ...
              <div class="txn-info">
                ...
                <span class="txn-date t-mono">
                  {{ formatTime(txn.created_at) }}
                  <span v-if="txn.category" class="txn-cat">· {{ txn.category }}</span>
                  <span v-if="txn.account" class="txn-cat">· {{ txn.account.name }}</span>
                </span>
              </div>
              <div class="txn-right">
                ...
                <span v-if="txn.cloudinary_url" class="receipt-dot" title="Receipt attached" />
              </div>
            </div>
          </div>
        </section>
        ...
        <!-- ── Budget ── -->
        <section class="dash-section sov-fade-up sov-fade-up--d3">
          <div class="section-hdr">
          ...
          <div v-if="budgets.length === 0" class="empty-card">
            ...
            <router-link to="/dashboard/budget" class="t-mono empty-link">Set one up →</router-link>
          </div>
            ...
            <div v-for="b in budgets" :key="b.id" class="budget-row">
              <div class="budget-row-top">
                <div class="budget-row-left">
                  ...
                  <span v-if="b.spent > b.total" class="budget-over-chip t-mono">OVER</span>
                </div>
                <span class="budget-amt t-mono">
                  <span :style="{ color: b.spent > b.total ? '#ef5350' : 'rgba(224,224,224,0.8)', fontFamily: 'var(--font-mono)' }">{{ formatAmount(b.spent) }}</span>
              ...
              <div class="sov-track" style="margin-top:7px">
                <div class="sov-track__fill" :style="{
                  width: Math.min((b.spent / b.total) * 100, 100) + '%',
                  background: b.spent > b.total ? '#ef5350' : b.color,
        ...
        <section class="dash-section sov-fade-up sov-fade-up--d3">
          <div class="section-hdr">
            ...
            <router-link to="/dashboard/goals" class="section-link t-mono">All →</router-link>
          </div>

          <div v-if="goals.length === 0" class="empty-card">
            <span class="t-mono empty-label">No goals yet</span>
            <router-link to="/dashboard/goals" class="t-mono empty-link">Add a goal →</router-link>
          </div>

          <div v-else class="goals-scroll">
            <div v-for="g in goals" :key="g.id" class="goal-card sov-card">
              <div class="goal-top">
                <span class="goal-icon" v-html="g.icon_svg" />
              ...
              <div class="sov-track" style="margin-top:10px">
                <div class="sov-track__fill" :style="{
                  ...
                  background: g.pct >= 100 ? 'var(--color-grn)' : (g.color ?? 'var(--color-ox)')
                }" />
              </div>
            </div>
        ...
        <!-- ── Recent Ledger ── -->
        <section class="dash-section sov-fade-up sov-fade-up--d4" style="padding-bottom:12px">
          <div class="section-hdr">
            <span class="t-mono section-tag">// RECENT LEDGER</span>
            <router-link to="/dashboard/transactions" class="section-link t-mono">All →</router-link>
          </div>

          <div v-if="recentTransactions.length === 0" class="empty-card">
            <span class="t-mono empty-label">No transactions yet</span>
            <router-link to="/dashboard/transactions?new=1" class="t-mono empty-link">Log your first →</router-link>
          </div>

          <div v-else class="txn-list sov-card">
            <div v-for="txn in recentTransactions" :key="txn.id" class="txn-row">
              <div class="txn-icon-wrap" :style="{ background: txnBg(txn) }">
                <span v-html="txnIconSvg(txn)" />
              </div>
              <div class="txn-info">
              ...
              <div class="txn-right">
                ...
                <span v-if="txn.cloudinary_url" class="receipt-dot" />
              </div>
            </div>
          </div>
        </section>
...
import { useTransactions, type ITransaction }     from '@/composables/useTransactions'
import { useLedger }                              from '@/composables/useLedger'

const { user } = useAuth()

// ── Shared composable state ───────────────────────────────────────────────────
const { activeAccounts, fetchAccounts } = useAccount()
const {
  fetchTransactions,
  todayTransactions,
  recentTransactions,
...
const {
  ...
  dsrColor, dsrLabel,
  budgetDaysLeft, overspentBudgets, savingsRate,
  fetchAll,
} = useLedger()
...
const hasNotif = computed(() =>
  (user.value && !user.value.is_verified) || overspentBudgets.value.length > 0
)

const activeCount = computed(() => activeAccounts.value.length)
const hasMoreAccounts = computed(() => activeCount.value > 4) // Assuming limit is 4 for display
...
function formatTime(iso: string): string {
  try { return new Date(iso).toLocaleTimeString('en-MY', { hour: '2-digit', minute: '2-digit' }) }
  catch { return '' }
}
...
function networkIcon(type: string): string {
  if (type === 'bank')
    return `<svg width="36" height="22" viewBox="0 0 36 22" fill="none"><circle cx="13" cy="11" r="9" fill="rgba(255,255,255,0.18)"/><circle cx="23" cy="11" r="9" fill="rgba(255,255,255,0.1)"/></svg>`
  if (type === 'ewallet')
    return `<svg width="36" height="22" viewBox="0 0 36 22" fill="none"><path d="M6 11 L18 4 L30 11 L18 18 Z" fill="rgba(255,255,255,0.12)"/><circle cx="18" cy="11" r="4" fill="rgba(255,255,255,0.15)"/></svg>`
...
function txnIconSvg(txn: Partial<ITransaction>): string {
  const color = txn.txn_type === 'income' ? '#4CAF50' : '#800020'
  if (txn.txn_type === 'income') {
    return `<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 9L6 5l3 3 4-5" stroke="${color}" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/></svg>`
  }
  return `<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5.5" stroke="${color}99" stroke-width="1.1"/><path d="M4 7.5c.5 1 1.3 1.5 2.5 1.5s2-.5 2.5-1.5" stroke="${color}99" stroke-width="1" stroke-linecap="round"/></svg>`
}

function txnBg(txn: Partial<ITransaction>): string {
  const color = txn.txn_type === 'income' ? '#4CAF50' : '#800020'
  return `${color}18`
}
...
.notif-btn { width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:rgba(224,224,224,0.05); border:1px solid var(--color-glass-bo); color:var(--color-gun); cursor:pointer; position:relative; transition:all .18s; }
...
/* ── Nudge ── */
.dash-nudge { display:flex; align-items:center; gap:8px; padding:10px 14px; margin-bottom:16px; background:var(--color-amb-lo); border:1px solid rgba(251,140,0,0.2); border-radius:var(--radius-sm); color:rgba(251,140,0,0.8); }
...
.nw-savings-rate { font-size:8px; letter-spacing:1.5px; color:var(--color-gun); }
...
/* ── Today section ── */
.today-hdr-left { display:flex; align-items:center; gap:10px; }
.today-live-badge { display:inline-flex; align-items:center; gap:6px; padding:3px 9px; background:var(--color-ox-lo); border:1px solid var(--color-ox-md); border-radius:100px; }
.today-net    { font-size:10.5px; letter-spacing:0.3px; }
.today-net--pos { color:var(--color-grn); }
.today-net--neg { color:#ef5350; }
.today-empty  { display:flex; align-items:center; justify-content:space-between; padding:16px 18px; gap:12px; }
.today-empty-inner { display:flex; align-items:center; gap:12px; }
.today-empty-icon { width:36px; height:36px; border-radius:10px; background:rgba(224,224,224,0.03); border:1px solid var(--color-glass-bo); display:flex; align-items:center; justify-content:center; flex-shrink:0; }
.txn-row--today { background:rgba(255,255,255,0.012); }
...
.budget-over-chip { font-size:7px; letter-spacing:1px; padding:2px 6px; background:var(--color-red-lo); border:1px solid rgba(176,0,32,0.22); border-radius:100px; color:#ef5350; }
...
.txn-cat   { opacity:0.65; }
.txn-right { display:flex; flex-direction:column; align-items:flex-end; gap:4px; flex-shrink:0; }
.txn-amt   { font-size:11px; letter-spacing:0.3px; }
.t-green   { color:var(--color-grn); }
.t-red     { color:#ef5350; }

/* Receipt indicator dot */
.receipt-dot { width:5px; height:5px; border-radius:50%; background:var(--color-gun); opacity:0.4; }
...
/* Pulse animation (mirrors animations.css) */
.sov-pulse { width:7px; height:7px; flex-shrink:0; background:var(--color-ox); border-radius:50%; box-shadow:0 0 6px var(--color-ox-hi); animation:sovPulse 2.5s ease-in-out infinite; }
@keyframes sovPulse {
  0%, 100% { box-shadow:0 0 4px var(--color-ox-hi); }
  50%       { box-shadow:0 0 14px var(--color-ox), 0 0 22px var(--color-ox-md); }
}
...
.bank-card--overflow {
  ...
  border: 1.5px dashed rgba(180, 20, 50, 0.25);
  box-shadow: none;
  align-items: center;
  justify-content: center;
  transition: border-color .2s, background .2s;
}
.bank-card--overflow:hover {
  border-color: var(--color-ox-md);
  background: var(--color-ox-lo);
}
.card-overflow-inner {
  display: flex;
...
.card-overflow-icon {
  ...
  border: 1px solid var(--color-ox-md);
  display: flex;
  align-items: center;
  justify-content: center;
  color: var(--color-ox);
...
/* ── Animations (local mirror of animations.css) ── */
.sov-fade-up { opacity:0; transform:translateY(14px); animation:sovFadeUp var(--duration-slow) var(--ease) forwards; }
.sov-fade-up--d1 { animation-delay:80ms; }
.sov-fade-up--d2 { animation-delay:160ms; }
.sov-fade-up--d3 { animation-delay:240ms; }
.sov-fade-up--d4 { animation-delay:320ms; }
@keyframes sovFadeUp { to { opacity:1; transform:translateY(0); } }
</style>